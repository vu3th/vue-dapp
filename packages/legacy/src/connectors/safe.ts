...
import { ConnectorNotFoundError, ProviderNotFoundError, ProviderRpcError } from './errors'
	...
	async getProvider() {
		if (!this.#provider) {
			...
			try {
				...
				if (typeof SafeAppProvider !== 'function' && typeof module.default.SafeAppProvider === 'function') {
	...
	async #getSafeSDK(): Promise<SafeAppsSDK> {
		...
		try {
			...
			if (
				...
				SafeAppsSDK = (SafeAppsSDK as unknown as { default: typeof SafeAppsSDK }).default
			}
		} catch (err: any) {
