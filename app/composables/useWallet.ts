...
import { AutoConnectError, ConnectError, Connector, ConnectorNotFoundError, MetaMaskConnector } from '@vue-dapp/core'
...
export function useWallet(options: useWalletOptions = { useEthers: true }) {
	...
	async function autoConnect(connectors: Connector[]) {
		...
		// dev
		const safe = connectors.find(conn => conn.name === 'safe') as any
